...
const DriverAcceptDetailScreen = ({route, navigation}) => {
    ...
    const startChat = async (userOrderId) => {
        /*  const params = {
            orderId: userOrderId,
          };*/
        ...
        const params = {
            ...
            driverOrderId: orderDetailInfo.driverOrderId,
            passerUserCode: orderDetailInfo.passengerId,
            driverUserCode: orderDetailInfo.driverId,
            createdDateTime: format(new Date(), "yyyy-MM-dd HH:mm:ss"),
            driverName: userInfo.userName,
            passerName: orderDetailInfo.userName,
        }
        createChatList(params).then(data=>{
            responseOperation(data.code, () => {
                    ...
                    {Status !== OrderStateEnum.CANCELLED && Status !== OrderStateEnum.COMPLETED && Status !== OrderStateEnum.DELIVERED && (
                        <View style={{position: "relative"}}>
                            {(Status === OrderStateEnum.PENDING) && (
                                <View>
                                    <Text>Arrive Before <Text fontWeight="bold"
                                                              color="#0000FF">{formatDate(new Date(Time))} </Text>For
                                        Pickup.</Text>
